...
#include "Multispecies.h"
  ...
  if (mode==MODE_DENS_TEMP) then
     ...
     do k = vecBegin, vecEnd
        ...
        if (tempRow(k) .LE. eos_smallt) then
     ...
     do k = vecBegin, vecEnd
        ...
        call Driver_abortFlash('Error: too many iterations in Helmholtz Eos')
...
70      continue           
     ...
     do k = vecBegin, vecEnd
        ...
        call Driver_abortFlash('Error: too many iteration in Eos')
...
170     continue
     ...
     if (myPE.EQ.MASTER_PE) print*,'Eos: the mode here is',mode
     call Logfile_stamp(eos_myPE, mode, "Eos: unknown mode ")
     call Driver_abortFlash('Error: unknown input in routine eos')
  end if
