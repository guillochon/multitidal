...
#include "Flash.h"
       ...
       do k = blkLimits(LOW, KAXIS), blkLimits(HIGH, KAXIS)
           ...
           do j = blkLimits(LOW, JAXIS), blkLimits(HIGH, JAXIS)
               ...
               do i = blkLimits(LOW, IAXIS), blkLimits(HIGH, IAXIS)
...
#ifdef VELX_VAR      
                ! momentum
                velx = solnData(VELX_VAR,i,j,k) + grv_obvec(4) - bvx
                lsum(2) = lsum(2) + solnData(DENS_VAR,i,j,k) * & 
                     &                                solnData(VELX_VAR,i,j,k)*dvol 
#endif
#ifdef VELY_VAR      
                vely = solnData(VELY_VAR,i,j,k) + grv_obvec(5) - bvy
                lsum(3) = lsum(3) + solnData(DENS_VAR,i,j,k) * & 
                     &                                solnData(VELY_VAR,i,j,k)*dvol
#endif
#ifdef VELZ_VAR      
                velz = solnData(VELZ_VAR,i,j,k) + grv_obvec(6) - bvz
                ...
                lsum(8) = lsum(8) + solnData(GPOT_VAR,i,j,k)/2.d0 * solnData(DENS_VAR,i,j,k)*dvol
#endif
                xx = bndBox(LOW,IAXIS) + (i-imin+0.5d0)*delta(IAXIS)
                ptx = xx - Xcm - grv_ptvec(1) + grv_obvec(1)
                pty = yy - Ycm - grv_ptvec(2) + grv_obvec(2)
                ptz = zz - Zcm - grv_ptvec(3) + grv_obvec(3)
                ptrad = sqrt(ptx**2.d0 + pty**2.d0 + ptz**2.d0)
                ...
                if (-solnData(GPOT_VAR,i,j,k) .ge. v2) then
                    ...
                    lsum(14) = lsum(14) + solnData(GPOT_VAR,i,j,k)/2.d0 * solnData(DENS_VAR,i,j,k)*dvol
    ...
    call MPI_Reduce (lsum, gsum, nGlobalSum, FLASH_REAL, MPI_SUM, MASTER_PE, MPI_COMM_WORLD, error)
