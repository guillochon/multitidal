subroutine Orbit_energy(blockCount, blockList)
    use Grid_interface, ONLY : Grid_getBlkIndexLimits, Grid_getBlkPtr, Grid_releaseBlkPtr,&
        Grid_getCellCoords, Grid_putPointData, Grid_getMinCellSize, Grid_getMyPE, Grid_getDeltas,&
        Grid_getBlkBoundBox
    ...
    use gr_mpoleData, ONLY : Xcm, Ycm, Zcm, oXcm, oYcm, oZcm, Mtot
    use Driver_data, ONLY : dr_dtOld
    use Simulation_data, ONLY : sim_sinkRadius
    implicit none
#include "constants.h"
#include "Flash.h"
#include "Flash_mpi.h"
    ...
    inner_rad = sim_sinkRadius*min_cell
        ...
        do k = blkLimits(LOW, KAXIS), blkLimits(HIGH, KAXIS)
            ...
            do j = blkLimits(LOW, JAXIS), blkLimits(HIGH, JAXIS)
                ...
                do i = blkLimits(LOW, IAXIS), blkLimits(HIGH, IAXIS)
                    !if (solnData(DENS_VAR,i,j,k) .lt. grv_thresh) cycle
                    xx = bndBox(LOW,IAXIS) + (i-imin+0.5)*delta(IAXIS)
                    x = xx - Xcm - grv_ptvec(1) + grv_obvec(1)
                    y = yy - Ycm - grv_ptvec(2) + grv_obvec(2)
                    z = zz - Zcm - grv_ptvec(3) + grv_obvec(3)
                    r = max(sqrt(x**2.d0 + y**2.d0 + z**2.d0), inner_rad**2.d0/sqrt(x**2.d0 + y**2.d0 + z**2.d0))
