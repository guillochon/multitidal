!!****if* source/Grid/GridSolvers/Multipole/gr_zoneOldPotential
!!
!! NAME
!!
!!  gr_zoneOldPotential
...
!! SYNOPSIS
!!
!!  gr_zoneOldPotential(real(in) :: xprime, 
!!                  real(IN) :: yprime,
!!                  real(IN) :: zprime,
!!                  real(OUT) :: potential)
!!
...
subroutine gr_zoneOldPotential (xprime, yprime, zprime, potential)
  ...
  use gr_mpoleData, ONLY : mpole_geometry, G_1DSPHERICAL,&
                         ...
                         mpole_mmax,mpole_lmax,costable,sintable,OldMoment,&
                         Legk1, Legk2, Even, Odd, Inner, Outer,&
                         rpower, rprinv, qmax, dsinvarr, rad
  ...
  !               qprime          The index q (for OldMoment()) at which the current
  !                                 cell falls
  ...
  if (qprime .gt. qmax .or. qprime .eq. -1) then
     write (str_buffer,*) 'gr_zoneOldPotential: desired q = ', qprime, & 
  ...
  Legnd1 = 1.
  potential = potential + & 
       (f1*OldMoment(qprmm1,Even,Inner,0,0) + & 
       f *OldMoment(qprime,Even,Inner,0,0))*rprinv(0) + & 
       (f1*OldMoment(qprime,Even,Outer,0,0) + & 
       f *OldMoment(qprmp1,Even,Outer,0,0))*rpower(0)
  ...
  if (mpole_lmax >= 1) then
     ...
     potential = potential + & 
          (f1*OldMoment(qprmm1,Even,Inner,1,0) + & 
          f *OldMoment(qprime,Even,Inner,1,0))*rprinv(1)*Legnd2 + & 
          (f1*OldMoment(qprime,Even,Outer,1,0) + & 
          f *OldMoment(qprmp1,Even,Outer,1,0))*rpower(1)*Legnd2
  endif
  ...
  do l = 2, mpole_lmax
     ...
     Legnd2 = Legndr
     potential = potential + & 
          (f1*OldMoment(qprmm1,Even,Inner,l,0) + & 
          f *OldMoment(qprime,Even,Inner,l,0))*rprinv(l)*Legndr + & 
          (f1*OldMoment(qprime,Even,Outer,l,0) + & 
          f *OldMoment(qprmp1,Even,Outer,l,0))*rpower(l)*Legndr
  enddo
  ...
  do m = 1, mpole_lmax
     ...
     potential = potential + & 
          ((f1*OldMoment(qprmm1,Even,Inner,m,m) + & 
          f *OldMoment(qprime,Even,Inner,m,m))*mtemp3 + & 
          (f1*OldMoment(qprime,Even,Outer,m,m) + & 
          f *OldMoment(qprmp1,Even,Outer,m,m))*mtemp4)*costable(m) + & 
          ((f1*OldMoment(qprmm1,Odd,Inner,m,m) + & 
          f *OldMoment(qprime,Odd,Inner,m,m))*mtemp3 + & 
          (f1*OldMoment(qprime,Odd,Outer,m,m) + & 
          f *OldMoment(qprmp1,Odd,Outer,m,m))*mtemp4)*sintable(m)
     ...
     if (mp1 .le. mpole_lmax) then
        ...
        potential = potential + & 
             ((f1*OldMoment(qprmm1,Even,Inner,mp1,m) + & 
             f *OldMoment(qprime,Even,Inner,mp1,m))*mtemp3 + & 
             (f1*OldMoment(qprime,Even,Outer,mp1,m) + & 
             f *OldMoment(qprmp1,Even,Outer,mp1,m))*mtemp4)*costable(m) + & 
             ((f1*OldMoment(qprmm1,Odd,Inner,mp1,m) + & 
             f *OldMoment(qprime,Odd,Inner,mp1,m))*mtemp3 + & 
             (f1*OldMoment(qprime,Odd,Outer,mp1,m) + & 
             f *OldMoment(qprmp1,Odd,Outer,mp1,m))*mtemp4)*sintable(m)
     endif
     ...
     do l = m+2, mpole_lmax
        ...
        potential = potential + & 
             ((f1*OldMoment(qprmm1,Even,Inner,l,m) + & 
             f *OldMoment(qprime,Even,Inner,l,m))*mtemp3 + & 
             (f1*OldMoment(qprime,Even,Outer,l,m) + & 
             f *OldMoment(qprmp1,Even,Outer,l,m))*mtemp4)*costable(m) + & 
             ((f1*OldMoment(qprmm1,Odd,Inner,l,m) + & 
             f *OldMoment(qprime,Odd,Inner,l,m))*mtemp3 + & 
             (f1*OldMoment(qprime,Odd,Outer,l,m) + & 
             f *OldMoment(qprmp1,Odd,Outer,l,m))*mtemp4)*sintable(m)
     enddo
  ...
  return
end subroutine gr_zoneOldPotential
