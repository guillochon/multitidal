...
!--locals-------------------------
    ...
    real, DIMENSION(numCells)    :: rhoav, uav, utav, uttav, eint, &
              ...
              &                   v, dvol, &
              &                   ograv, ograv2, hgrav, ptgrav, optgrav
    ...
    double precision, dimension(numCells) :: dg, hdg, ptdg
    double precision, dimension(3) :: tgrid, coef
...
!  real :: pres_jump
    ...
    if (useGravity) then
       ...
       !tgrid(1) = 0.d0
       !tgrid(2) = dr_dtOld2
       !tgrid(3) = dr_dtOld2 + dt_old
       !call fdcoef(3, 3, tgrid(3), tgrid, coef)
       !dg2 = (coef(1)*ograv2+coef(2)*ograv+coef(3)*grav)
       !call fdcoef(2, 3, tgrid(3), tgrid, coef)
       !hdg = 0.5*dt*(coef(1)*ograv2+coef(2)*ograv+coef(3)*grav) + 0.125d0*dt**2.d0*dg2
       !dg = dt*(coef(1)*ograv2+coef(2)*ograv+coef(3)*grav) + 0.5d0*dt**2.d0*dg2

       dg = dt/dt_old*(grav - ograv)
       hgrav = grav + 0.5d0*(ptgrav + optgrav) + 0.5d0*dg
       ngrav = grav + dg + ptgrav
       grav = grav + optgrav
       ...
       call Gravity_accelOneRow (pos, xyzswp, blockID, numIntCells8,grav,ptgrav)
       hgrav = grav + ptgrav
       ngrav = grav + ptgrav
#endif
...
! Obtain PPM interpolation coefficients.
    call intrfc(numIntCells,numCells, guard,&
         &      rho,u,ut,utt,p, &
         &      rhol,rhor,ul,ur, &
         &      utl,utr,uttl,uttr, &
         &      pl,pr,vl,vr,gamcl, &
         ...
         &      xnl,xnr,v,primaryDx, primaryCoord)
